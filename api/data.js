...
module.exports = async (req, res) => {
  ...
  // ?mode=status returns fetch progress + quota (replaces fetch-status.js)
  if (req.query.mode === 'status') {
    try {
      const supabase = getSupabaseClient();
      return await handleFetchStatus(req, res, supabase);
    } catch (e) {
...
// ── FETCH STATUS (merged from fetch-status.js) ──
// GET /api/data?mode=status
async function handleFetchStatus(req, res, supabase) {
  ...
  res.status(200).json({
    ...
    video_mapped: counts.searched + counts.fetched,
    pending_search: counts.pending,
    today: {
