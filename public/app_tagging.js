...
async function runTagging() {
  ...
  const allUntagged = STATE.comments.filter(c => !c.tag);

  if (allUntagged.length === 0) return;

  // Priority order for tagging:
  // P1: E-com (Amazon/Flipkart) of models with ZERO existing tags
  // P2: E-com of models that have some tags but new untagged exist
  // P3: YouTube of models with ZERO existing tags
  // P4: YouTube of models with some tags but new untagged exist
  const taggedModelIds = new Set(
    STATE.comments.filter(c => c.tag).map(c => c.model_id)
  );
  const p1=[], p2=[], p3=[], p4=[];
  allUntagged.forEach(c => {
    const isEcom = c.source === 'Amazon' || c.source === 'Flipkart';
    const hasExistingTags = taggedModelIds.has(c.model_id);
    if      (isEcom  && !hasExistingTags) p1.push(c);
    else if (isEcom  &&  hasExistingTags) p2.push(c);
    else if (!isEcom && !hasExistingTags) p3.push(c);
    else                                  p4.push(c);
  });
  const untagged = [...p1, ...p2, ...p3, ...p4];
