...
async function fetchVideoCandiates(modelId) {
  ...
  try {
    const data = await apiPost('/api/videos?action=search', { model_id: modelId });
...
async function saveVideoSelection(modelId) {
  ...
  try {
    ...
    await apiPost('/api/videos?action=save', { model_id: modelId, videos: selectedVideos });
...
async function addManualVideo(modelId) {
  ...
  try {
    await apiPost('/api/videos?action=save', { model_id: modelId, videos: allVideos });
    urlInput.value = '';
    const fresh = await apiGet('/api/data');
    STATE.comments = fresh.comments;
    STATE.videoMap = fresh.videoMap;
